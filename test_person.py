...
def drawcai(img, boxes, classes, scores, category_index):
    ...
    for i in range(detectn):
        ...
        cv2.rectangle(img, (label_xmin - 1, label_ymin - 1),
                      (label_xmax + 1, label_ymax + 1), label_background_color, -1)
...
test_folder_path = './datasets/test/'
...
for img_path in imglist:
    ...
    drawcai(cvimage, detections['detection_boxes'][0].numpy(), detections['detection_classes'][0].numpy(
    ).astype(np.uint32), detections['detection_scores'][0].numpy(), category_index)
